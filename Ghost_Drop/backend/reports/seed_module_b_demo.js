...
async function main() {
  ...
  await query("DELETE FROM portfolio_entries WHERE vault_id IN (SELECT vault_id FROM vaults WHERE outer_token = ?)", [DEMO.outerToken]);
  await query("DELETE FROM inner_tokens WHERE vault_id IN (SELECT vault_id FROM vaults WHERE outer_token = ?)", [DEMO.outerToken]);
  await query("DELETE FROM vaults WHERE outer_token = ?", [DEMO.outerToken]);
