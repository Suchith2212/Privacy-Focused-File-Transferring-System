"""
Benchmark the GhostDrop-oriented Module A indexes against a brute-force baseline.
...
DEFAULT_RUNS = 1
SUMMARY_PATH = INTEGRATION / "benchmark_ghostdrop_paths_summary.md"
RESULTS_JSON = INTEGRATION / "benchmark_ghostdrop_paths.json"
...
def benchmark_once(base_snapshot: Dict[str, List[Dict[str, object]]], size: int) -> Dict[str, float]:
    ...
    bpt = GhostDropIndexManager(engine="bplustree", order=16)
    brute = GhostDropIndexManager(engine="bruteforce", order=16)
...
def plot_speedups(results: List[Dict[str, float]]) -> None:
    ...
    ax.set_title("GhostDrop Path Speedup Ratios", fontsize=16, fontweight="bold", pad=16)
    ax.set_xlabel("Dataset size (rows)", fontsize=11)
    ...
    fig.savefig(INTEGRATION / "path_speedup_benchmark.png", dpi=220, bbox_inches="tight")
    plt.close(fig)
...
def plot_dashboard(results: List[Dict[str, float]]) -> None:
    ...
    fig.suptitle("Module A GhostDrop Path Benchmark Dashboard", fontsize=18, fontweight="bold", y=0.98)
    fig.text(0.5, 0.945, f"{len(results)} points from real exported GhostDrop data", ha="center", fontsize=10, color="#4b5563")
...
def save_outputs(results: List[Dict[str, float]], source_path: Path, runs: int) -> None:
    ...
    lines = [
        "# GhostDrop Path Benchmark Summary",
        "",
        "This benchmark compares the Module A B+ Tree wrapper against the brute-force baseline on GhostDrop-shaped access paths derived from the exported project snapshot.",
        "",
