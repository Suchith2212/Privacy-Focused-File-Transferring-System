...
def plot_random_workload(data):
    ...
    path = EVIDENCE / "random_workload_benchmark.png"
    fig.savefig(path, dpi=220, bbox_inches="tight")
    plt.close(fig)
    return path.name
...
def plot_speedup(data):
    ...
    path = EVIDENCE / "speedup_comparison.png"
    fig.savefig(path, dpi=220, bbox_inches="tight")
    plt.close(fig)
    return path.name
...
def plot_memory_usage(memory_data):
    ...
    path = EVIDENCE / "retained_memory_after_insert.png"
    fig.savefig(path, dpi=220, bbox_inches="tight")
    plt.close(fig)
    return path.name
...
def plot_memory_peaks(memory_data):
    ...
    path = EVIDENCE / "peak_python_allocations.png"
    fig.savefig(path, dpi=220, bbox_inches="tight")
    plt.close(fig)
    return path.name
...
def plot_memory_after_delete(memory_data):
    ...
    path = EVIDENCE / "retained_memory_after_delete.png"
    fig.savefig(path, dpi=220, bbox_inches="tight")
    plt.close(fig)
    return path.name
...
def plot_memory_dashboard(memory_data):
    ...
    path = EVIDENCE / "memory_dashboard.png"
    fig.savefig(path, dpi=220, bbox_inches="tight")
    plt.close(fig)
    return path.name
...
def main():
    ...
    plots = {
        ...
        "insertion": plot_metric(RESULTS, "bpt_insert_ms", "bf_insert_ms", "Insertion Time", "insertion_benchmark.png"),
        "search": plot_metric(RESULTS, "bpt_search_ms", "bf_search_ms", "Search Time", "search_benchmark.png"),
        "deletion": plot_metric(RESULTS, "bpt_delete_ms", "bf_delete_ms", "Deletion Time", "deletion_benchmark.png"),
        "range": plot_metric(RESULTS, "bpt_range_ms", "bf_range_ms", "Selective Range Query Time", "range_benchmark.png"),
    ...
    payload = {
        "metadata": {
            ...
            "memory_measurement": "retained_memory_after_insert.png reports retained structure size after insert using a recursive object-size walk",
            "memory_peak_measurement": "peak_python_allocations.png reports tracemalloc peak Python allocations during build",
            "memory_delete_measurement": f"retained_memory_after_delete.png reports retained structure size after deleting {int(MEMORY_DELETE_FRACTION * 100)}% of keys",
    ...
    results_path = EVIDENCE / "detailed_benchmark_results.json"
