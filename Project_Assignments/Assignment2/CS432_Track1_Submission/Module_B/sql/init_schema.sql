...
SET @sql = IF(
  EXISTS(
    ...
    WHERE table_schema = DATABASE()
      ...
      AND index_name = 'idx_vault_expiry'
  ),
  'SELECT 1',
...
SET @sql = IF(
  EXISTS(
    ...
    WHERE table_schema = DATABASE()
      ...
      AND index_name = 'idx_files_vault_status'
  ),
  'SELECT 1',
...
SET @sql = IF(
  EXISTS(
    ...
    WHERE table_schema = DATABASE()
      ...
      AND index_name = 'idx_portfolio_vault_owner_status'
  ),
  'SELECT 1',
...
SET @sql = IF(
  EXISTS(
    ...
    WHERE table_schema = DATABASE()
      AND table_name = 'portfolio_entries'
      AND index_name = 'idx_portfolio_vault_status'
  ),
  'SELECT 1',
...
-- Optional hard-fail trigger intentionally omitted from the portable setup script.
-- The submission's primary unauthorized-modification detection path is the
-- application-side integrity hash validation exposed through /api/security/unauthorized-check
-- and enforced on portfolio reads/updates. Some managed/local MySQL configurations
-- reject CREATE TRIGGER without elevated binary logging privileges, which would make
-- the setup script fail even though the assignment-facing tamper detection remains functional.
